...
# Import custom modules 
from normalizer import MinMaxNormalizer
# NEW: Import the new UltimatePINN instead of the old PINNModel
...
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print(f"Using device: {device} (AIKosh Environment)")

# ==========================================
# 1. AIKOSH CSV DATA LOADING & KDTREE
# ==========================================
csv_path = "../aikosh_unified_data.csv" 
print(f"Loading data from {csv_path}...")

df = pd.read_csv(csv_path)

wall_df = df[df['is_wall'] == 1]
wall_coords = wall_df[['x', 'y', 'z']].values

print(f"Building KDTree with {len(wall_coords):,} wall boundary points...")
tree = cKDTree(wall_coords)

all_coords = df[['x', 'y', 'z']].values
distances, _ = tree.query(all_coords)

X_np = df[['x', 'y', 'z', 't']].values
X_np = np.column_stack((X_np, distances)) 

U_np = df[['u', 'v', 'w']].values
P_np = df[['p']].values
P_np = np.nan_to_num(P_np, nan=0.0) 
...
X_norm = spatial_normalizer.encode(X).to(device)
U_norm = velocity_normalizer.encode(U).to(device)
P_norm = pressure_normalizer.encode(P).to(device)
...
# ==========================================
# 3. MODEL SETUP & CHECKPOINTING
# ==========================================
# NEW: Instantiate the UltimatePINN instead of the old MLP structure
...
for epoch in range(start_epoch, Epoches + 1):
    ...
    for batch_idx, (x_batch, u_batch, p_batch) in enumerate(train_loader):
        ...
        batch_ones = ones_wrapper[:current_batch_size]
        ...
        # Losses
        loss_data = F.mse_loss(u_pred, u_batch)
        loss_physics = get_physics_loss(prediction, x_batch, F.softplus(model.viscosity), batch_ones, scales=scales)
        
        # Physical Penalties
        p_real = p_norm * scales['p']
        P_MAX, P_MIN = 8000.0, 0.0   
        VISC_MAX, VISC_MIN = 0.006, 0.002
    ...
    # ==========================================
    # 5. MEMORY-SAFE BATCHED EVALUATION
    # ==========================================
    if epoch % 100 == 0:
        elapsed = time.time() - running_time
        model.eval()
        ...
        with torch.no_grad():
            # Training Accuracy (Velocity)
            train_err_sq, train_tgt_sq = 0.0, 0.0
            for x_b, u_b, _ in train_loader:
                pred_b = model(x_b)
                u_pred_b = pred_b[:, 0:3]
            ...
            # Validation Accuracy (Pressure)
            val_err_sq, val_tgt_sq = 0.0, 0.0
            for x_v, _, p_v in val_loader:
                pred_v = model(x_v)
                p_pred_v = pred_v[:, 3:4]
                val_err_sq += torch.sum((p_pred_v - p_v)**2).item()
                val_tgt_sq += torch.sum((p_v)**2).item()
                
            val_acc = (1.0 - (val_err_sq**0.5) / (val_tgt_sq**0.5 + 1e-8)) * 100.0

        current_mu = F.softplus(model.viscosity).item()
        avg_total = total_loss / max(1, len(train_loader))

        print(f"Epoch {epoch} | Loss: {avg_total:.4f} | Train Acc (U): {train_acc:.2f}% | Val Acc (P): {val_acc:.2f}% | Visc: {current_mu:.5f} | Time: {elapsed:.1f}s")
...
with torch.no_grad():
    for x_test, u_test, _ in test_loader:
        test_pred = model(x_test)
        u_test_pred = test_pred[:, 0:3]
