...
from SIREN_model import SIREN_PINN as PINN
...
model = PINN(in_features=5, hidden_features=128, out_features=4).to(DEVICE)
...
for epoch in range(start_epoch, EPOCHS):
    ...
    if epoch % 100 == 0:
        model.eval()
        with torch.no_grad():
            # Quick Training Accuracy (Velocity) using the last batch to save time
            eval_preds = model(x_batch)
            eval_vel = eval_preds[:, 0:3]
            train_mse, train_acc = calculate_metrics(eval_vel, v_batch)
            
            # Full Validation Pass for Boundary Pressure
            val_err_sq, val_tgt_sq = 0.0, 0.0
            for x_v, _, p_v, _, mask_v in val_loader:
                if mask_v.any():
                    # Only calculate on the boundary points to measure wall constraints
